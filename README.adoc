...
When checking whether an `ActiveRecord::Relation` is empty, prefer `any?`/`empty?` (or `none?`) over `present?`/`blank?`. If the relation is loaded, both variants are equally fast, but if not, the former uses an efficient `EXISTS`/`SELECT 1 … LIMIT 1` query while the latter loads all matching records into memory. (Note: `any?` is only efficient without a block.)
